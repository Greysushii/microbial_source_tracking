...
class _HistoryPageState extends State<HistoryPage> {
  ...
  List<String> cachedUserOptions = []; // this will store initialized filtering options
  ...
  PlatformFile? pickedFile;
    ...
    Future selectFile() async {
      final result = await FilePicker.platform.pickFiles(); // for selecting a file from local storage
    ...
    double uploadProgress = 0.0; // upload progress for the progress bar, which increases in correlation to bytes transfered to the database
  
  
    Future uploadFile() async {

      bool serviceEnabled;
      LocationPermission permission; // permissions for accessing device location for android/ios. to record latitude/longitutde metadata
      ...
      if (waterSource != null) {
        ...
        setState(() {
          ...
          task.snapshotEvents.listen((TaskSnapshot snapshot) {
            setState(() {
              uploadProgress = snapshot.bytesTransferred / snapshot.totalBytes; // progress bar for uploading file
            });
          });
          ...
          QuerySnapshot querySnapshot = await FirebaseFirestore.instance.collection('users').where('email', isEqualTo: currentUserEmail).get(); // finds the user document corresponding to the currently-logged-in user
          ...
          final image = <String, dynamic>{ // uploaded document and its fields
    ...
    Future getWaterSource() async { // function for getting the water source AKA lake name from the user. needed bc GPS will provide latitude/longitude but not actual lake name. 
    ...
    Future showSelectedFile() async { // shows selected file for preview before user decides to upload it to make sure its the right file
      if (pickedFile != null) {
    ...
    List<String> selectedLakes = []; // these variables hold the user's selected filtering options
    List<String> selectedUsers = [];
 
    Future fetchUserOptions() async { // see comment on Line 32
  ...
  Future fetchLakeOptions() async {
    ...
    bool showAllDates = true; // changes based on whether a date is selected or not for data filtering
  
 
  
  Stream<QuerySnapshot> getDocumentStream() { // StreamBuilder (located lower in this file) listens to Stream (this function). Stream changes the documents viewable on the user's feed based on their selected filters. 
    ...
    if (!showAllDates) {
      collectionQuery = collectionQuery.where('uploadedDate', isGreaterThanOrEqualTo: start).where('uploadedDate', isLessThan: end); // Handles date filtering
    }
    
    if (selectedLakes.isNotEmpty) {
      collectionQuery = collectionQuery.where('lake', whereIn: selectedLakes); // handles location filtering
    }

    if (selectedUsers.isNotEmpty) {
      collectionQuery = collectionQuery.where('uploader\'s email', whereIn: selectedUsers); // handles User filtering
    }
            ...
            children: [
              ...
              Row(
                ...
                children: [
                    ...
                    child: Row(
                      children: [
                        TextButton(
                          ...
                          onPressed: () async {
                            DateTime? pickedDate = await showDatePicker( // Selecting Date filtering option opens up calendar to pick date
                              context: context,
                  ...
                  TextButton(
                    child: Text('Reset'), //Restting filters sets everything back to default
                    onPressed: () {
                      setState(() {
                        showAllDates = true;
                        selectedLakes = [];
                ...
                builder: (context, snapshot) {
                  ...
                  return Column(
                    children: documents.map((doc) {
                      ...
                      return Dismissible( // Swipe-to-Delete: Each item can be swiped to delete it
                        key: UniqueKey(), 
                        ...
                        child: Card(
                          child: ListTile(
                            ...
                            Navigator.push(
                                ...
                                MaterialPageRoute(
                                    builder: ((context) =>
                                        HistoryPageView(doc: doc)))); // routes to another page to display the selected file's information
                          },
                            trailing: IconButton(
                              ...
                              onPressed: () {
                                showDialog(
                                  ...
                                  builder: (BuildContext context) {
                                    ...
                                    String oldTitle = doc['title']; // Changing the 'lake' field and title of a document/file, in case of an error or typo. 
                                    String datePart = "";
                                    bool foundDatePart = false;

                                    for (int i = 0; i < oldTitle.length; i++) {
                                      if (oldTitle[i].contains(RegExp(r'[0-9]'))) { // Since the title of each file is "Lake Name MM-DD-YYYY", a concatenation of 2 strings (lake string and date string), but only the Lake string needs to be edited, 
                                        foundDatePart = true;                       // due to the original date needing to stay the same, this iterates the string until a numerical value is reached, after which the date portion of the title is saved and combined with the user-inputted new lake/location name. 
                                      }
                                    ...
                                    return AlertDialog(
                                      title: Text('Edit sample location:'), // user input for editing file location
                                      content: TextField(
                                        ...
                                        TextButton(
                                          ...
                                          onPressed: () async {
                                            ...
                                            if (newLocation.isNotEmpty) {
                                              ...
                                              ScaffoldMessenger.of(context).showSnackBar(
                                                const SnackBar(
                                                  content: Text('Sample source and file name updated!'),
                                                ),
                                              );
                                            } else {
                                              ScaffoldMessenger.of(context).showSnackBar(
                                                const SnackBar(
                                                  content: Text('Please enter a valid sample source.'),
                                                ),
                                              );
                                            }
                                          },
